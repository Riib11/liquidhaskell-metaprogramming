...
spliceExp (Auto {hints, depth} : instrs) = do
  do
    env <- get
    return $! unsafePerformIO $ when _DUMP_AUTO $ putStrLn $ show env
  e <- do
    env <- get
...
genNeutrals goal gas = do
  ...
  return $! unsafePerformIO $ when _DUMP_AUTO $ putStrLn $ pprint es
  pure es
        ...
        then do
          if List.null alphas
            then fail "impossible: cannot recurse with 0 arguments"
              ...
              return $! unsafePerformIO $ when _DUMP_AUTO $ putStrLn $ pprint (foldl AppE r <$> argss)
